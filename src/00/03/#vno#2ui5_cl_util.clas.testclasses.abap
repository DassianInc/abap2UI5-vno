...
CLASS ltcl_unit_test_abap_api IMPLEMENTATION.
  ...
  METHOD test_raise_error.
    ...
    TRY.
      ...
      CATCH /vno/2ui5_cx_util_error INTO DATA(lx).
...
CLASS ltcl_unit_test IMPLEMENTATION.
  METHOD test_boolean_check.
    ...
    cl_abap_unit_assert=>assert_true( /vno/2ui5_cl_util=>boolean_check_by_data( lv_bool ) ).

    lv_bool = xsdbool( 1 = 2 ).
    cl_abap_unit_assert=>assert_true( /vno/2ui5_cl_util=>boolean_check_by_data( lv_bool ) ).

    cl_abap_unit_assert=>assert_true( /vno/2ui5_cl_util=>boolean_check_by_data( abap_true ) ).

    cl_abap_unit_assert=>assert_true( /vno/2ui5_cl_util=>boolean_check_by_data( abap_false ) ).
  ...
  METHOD test_sql_get_by_string.
    ...
    DATA(lv_test) = ``.
    DATA(ls_sql) = /vno/2ui5_cl_util=>filter_get_sql_by_sql_string( lv_test ) ##NEEDED.

  ENDMETHOD.

  METHOD test_create.

    DATA(lo_test) = NEW /vno/2ui5_cl_util( ) ##NEEDED.

  ENDMETHOD.

  METHOD test_rtti_get_classname_by_ref.

    DATA(lo_test) = NEW /vno/2ui5_cl_util( ).
    DATA(lv_name) = /vno/2ui5_cl_util=>rtti_get_classname_by_ref( lo_test ).
    cl_abap_unit_assert=>assert_equals( exp = `Z2UI5_CL_UTIL`
                                        act = lv_name ).

    DATA(lo_test2) = NEW ltcl_test_app( ).
    DATA(lv_name2) = /vno/2ui5_cl_util=>rtti_get_classname_by_ref( lo_test2 ).
  ...
  METHOD test_check_bound_a_not_inital.
    ...
    cl_abap_unit_assert=>assert_true( /vno/2ui5_cl_util=>check_bound_a_not_inital( lr_test ) ).
    CLEAR lv_test.
    cl_abap_unit_assert=>assert_false( /vno/2ui5_cl_util=>check_bound_a_not_inital( lr_test ) ).
    CLEAR lr_test.
    cl_abap_unit_assert=>assert_false( /vno/2ui5_cl_util=>check_bound_a_not_inital( lr_test ) ).
  ...
  METHOD test_check_unassign_inital.
    ...
    cl_abap_unit_assert=>assert_false( /vno/2ui5_cl_util=>check_unassign_inital( lr_test ) ).

    CLEAR lv_test.
    cl_abap_unit_assert=>assert_true( /vno/2ui5_cl_util=>check_unassign_inital( lr_test ) ).
  ...
  METHOD rtti_check_ref_data.
    ...
    cl_abap_unit_assert=>assert_true( /vno/2ui5_cl_util=>rtti_check_ref_data( lr_data ) ).

    cl_abap_unit_assert=>assert_false( /vno/2ui5_cl_util=>rtti_check_ref_data( lv_test ) ).
  ...
  METHOD conv_copy_ref_data.
    ...
    DATA(lr_test2) = /vno/2ui5_cl_util=>conv_copy_ref_data( lr_data ).
  ...
  METHOD test_boolean_abap_2_json.
    ...
    cl_abap_unit_assert=>assert_equals( exp = `{ABCD}`
                                        act = /vno/2ui5_cl_util=>boolean_abap_2_json( `{ABCD}` ) ).

  ENDMETHOD.

  METHOD test_time_get_timestampl.

    DATA(lv_time) = /vno/2ui5_cl_util=>time_get_timestampl( ).

    DATA(lv_time2) = /vno/2ui5_cl_util=>time_substract_seconds( time    = lv_time
                                                            seconds = 60 * 60 * 4 ).
  ...
  METHOD test_time_substract_seconds.
    ...
    DATA(lv_time) = /vno/2ui5_cl_util=>time_get_timestampl( ).
    DATA(lv_time2) = /vno/2ui5_cl_util=>time_get_timestampl( ).
  ...
  METHOD test_c_trim.
    ...
    cl_abap_unit_assert=>assert_equals( exp = `JsadfHHs`
                                        act = /vno/2ui5_cl_util=>c_trim( ` JsadfHHs  ` ) ).
  ...
  METHOD test_c_trim_lower.
    ...
    cl_abap_unit_assert=>assert_equals( exp = `jsadfhhs`
                                        act = /vno/2ui5_cl_util=>c_trim_lower( ` JsadfHHs  ` ) ).
  ...
  METHOD test_c_trim_upper.
    ...
    cl_abap_unit_assert=>assert_equals( exp = `JSADFHHS`
                                        act = /vno/2ui5_cl_util=>c_trim_upper( ` JsadfHHs  ` ) ).

  ENDMETHOD.

  METHOD test_func_get_user_tech.

    cl_abap_unit_assert=>assert_equals( exp = /vno/2ui5_cl_util=>context_get_user_tech( )
                                        act = sy-uname ).

    cl_abap_unit_assert=>assert_not_initial( /vno/2ui5_cl_util=>context_get_user_tech( ) ).
  ...
  METHOD test_x_check_raise.
    ...
    TRY.
        /vno/2ui5_cl_util=>x_check_raise( xsdbool( 1 = 1 ) ).
        cl_abap_unit_assert=>fail( ).
      CATCH cx_root.
    ENDTRY.

    TRY.
        /vno/2ui5_cl_util=>x_check_raise( xsdbool( 1 = 3 ) ).
      CATCH cx_root.
        cl_abap_unit_assert=>fail( ).
    ENDTRY.
  ...
  METHOD test_trans_json_any_2__w_struc.
    ...
    cl_abap_unit_assert=>assert_equals( exp = `{"selected":false,"title":"test","value":""}`
                                        act = /vno/2ui5_cl_util=>json_stringify( ls_row ) ).

  ENDMETHOD.

  METHOD test_url_param_create_url.

    DATA(lt_param) = /vno/2ui5_cl_util=>url_param_get_tab(
                         `https://url.com/rvice_for_ui?sap-client=100&app_start=z2ui5_cl_app_hello_world` ).
    DATA(lv_url) = /vno/2ui5_cl_util=>url_param_create_url( lt_param ).
  ...
  METHOD test_url_param_get.
    ...
    DATA(lv_param) = /vno/2ui5_cl_util=>url_param_get(
                         val = `app_start`
  ...
  METHOD test_url_param_set.
    ...
    DATA(lv_param) = /vno/2ui5_cl_util=>url_param_set(
                         name  = `app_start`
  ...
  METHOD test_x_check_raise_not.
    ...
    TRY.
        /vno/2ui5_cl_util=>x_check_raise( xsdbool( 1 = 2 ) ).
      CATCH /vno/2ui5_cx_util_error.
        cl_abap_unit_assert=>fail( ).
    ENDTRY.
  ENDMETHOD.
  ...
  METHOD test_rtti_get_type_name.
    ...
    DATA(lv_name) = /vno/2ui5_cl_util=>rtti_get_type_name( lv_xsdbool ).
  ...
  METHOD test_rtti_get_type_kind.
    ...
    DATA(lv_type_kind) = /vno/2ui5_cl_util=>rtti_get_type_kind( lv_string ).
    ...
    lv_type_kind = /vno/2ui5_cl_util=>rtti_get_type_kind( lr_string ).
  ...
  METHOD test_rtti_check_type_kind.
    ...
    DATA(lv_string) = VALUE string( ).
    DATA lr_string TYPE REF TO string.
    cl_abap_unit_assert=>assert_false( /vno/2ui5_cl_util=>rtti_check_type_kind_dref( lv_string ) ).

    CREATE DATA lr_string.
    cl_abap_unit_assert=>assert_true( /vno/2ui5_cl_util=>rtti_check_type_kind_dref( lr_string ) ).
  ...
  METHOD test_rtti_get_t_attri_by_obj.
    ...
    DATA(lt_attri) = /vno/2ui5_cl_util=>rtti_get_t_attri_by_oref( lo_obj ).
  ...
  METHOD test_rtti_get_t_comp_by_struc.
    ...
    DATA(lt_comp) = /vno/2ui5_cl_util=>rtti_get_t_attri_by_any( ls_row ).
  ...
  METHOD test_trans_xml_any_2__w_obj.
    ...
    DATA(lo_obj) = NEW ltcl_test_app( ).
    DATA(lv_xml) = /vno/2ui5_cl_util=>xml_stringify( lo_obj ).
  ...
  METHOD test_trans_xml_2_any__w_obj.
    ...
    CLEAR lo_obj.
    /vno/2ui5_cl_util=>xml_parse( EXPORTING xml = lv_xml
  ...
  METHOD test_trans_xml_any_2__w_data.
    ...
    DATA(lv_xml) = /vno/2ui5_cl_util=>xml_stringify( ls_row ).
  ...
  METHOD test_c_trim_horizontal_tab.
    ...
    IF /vno/2ui5_cl_util=>c_trim( |{ cl_abap_char_utilities=>horizontal_tab }|
                                && |JsadfHHs|
  ...
  METHOD test_get_token_t_by_r_t.
    ...
    DATA(lt_range) = VALUE /vno/2ui5_cl_util=>ty_t_range( ( sign = 'I' option = 'EQ' low = `table` high = `` )
     ).

    DATA(lt_result) = /vno/2ui5_cl_util=>filter_get_token_t_by_range_t( lt_range ).

    DATA(lt_exp) = VALUE /vno/2ui5_cl_util=>ty_t_token(
  ...
  METHOD test_rtti_get_t_attri_by_incl.
    ...
    DATA(lt_attri) = /vno/2ui5_cl_util=>rtti_get_t_attri_by_include( CAST #( lo_datadescr ) ).
