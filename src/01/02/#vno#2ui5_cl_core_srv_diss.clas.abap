CLASS /vno/2ui5_cl_core_srv_diss DEFINITION
  PUBLIC FINAL
  CREATE PUBLIC.
...
CLASS /vno/2ui5_cl_core_srv_diss IMPLEMENTATION.
  METHOD constructor.
  ...
  METHOD create_new_entry.
    ...
    result-name = name.
    DATA(lo_model) = NEW /vno/2ui5_cl_core_srv_attri( attri = mt_attri
                                                  app   = mo_app ).
  ...
  METHOD diss_dref.
    ...
    IF /vno/2ui5_cl_util=>check_unassign_inital( ir_attri->r_ref ).
      RETURN.
    ENDIF.

    DATA(lr_ref) = /vno/2ui5_cl_util=>unassign_data( ir_attri->r_ref ).
  ...
  METHOD diss_oref.
    ...
    DATA(lr_ref) = /vno/2ui5_cl_util=>unassign_object( ir_attri->r_ref ).
  ...
  METHOD main_init.
    ...
    IF mt_attri->* IS NOT INITIAL.
      LOOP AT mt_attri->* TRANSPORTING NO FIELDS
           WHERE bind_type <> /vno/2ui5_if_core_types=>cs_bind_type-one_time.
      ENDLOOP.
      IF sy-subrc = 0.
        RETURN.
      ENDIF.
    ...
    DATA(ls_attri) = VALUE /vno/2ui5_if_core_types=>ty_s_attri( r_ref = REF #( mo_app ) ).
  ...
  METHOD main_run.
    ...
    LOOP AT mt_attri->* REFERENCE INTO DATA(lr_attri)
         WHERE     check_dissolved  = abap_false
               AND bind_type       <> /vno/2ui5_if_core_types=>cs_bind_type-one_time.
